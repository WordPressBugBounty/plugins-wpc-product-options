...
?>
    ...
    <div class="wpcpo-item-line">
        <label>
            <input type="checkbox" value="1"
                   name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][enable_limit]' ); ?>" <?php checked( $this->get_field_value( 'enable_limit' ), '1' ); ?>> <?php esc_html_e( 'Limit', 'wpc-product-options' ); ?>
            <div class="checkbox-show">
                <input type="number" name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][min]' ); ?>"
                       value="<?php echo esc_attr( $this->get_field_value( 'min' ) ); ?>" placeholder="min" min="0"/>
                <span>—</span>
                <?php if ( $type === 'number' ) { ?>
                    <input type="number" name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][step]' ); ?>"
                           value="<?php echo esc_attr( $this->get_field_value( 'step' ) ); ?>" placeholder="step"
                           min="0"
                           step="any"/>
                    <span>—</span>
                <?php } ?>
                <input type="number" name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][max]' ); ?>"
                       value="<?php echo esc_attr( $this->get_field_value( 'max' ) ); ?>" placeholder="max" min="0"/>
    ...
    <div class="wpcpo-item-line">
        <label>
            <input type="checkbox" value="1"
                   name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][default_value]' ); ?>" <?php checked( $this->get_field_value( 'default_value' ), '1' ); ?>> <?php esc_html_e( 'Default value', 'wpc-product-options' ); ?>
            <?php if ( $type === 'number' ) { ?>
                <input type="number" class="checkbox-show" min="0" step="any"
                       name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][value]' ); ?>"
                       value="<?php echo esc_attr( $this->get_field_value( 'value' ) ); ?>"/>
            <?php } else { ?>
                <input type="text" class="input-block checkbox-show"
                       name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][value]' ); ?>"
                       value="<?php echo esc_attr( $this->get_field_value( 'value' ) ); ?>"/>
            <?php } ?>
        </label>

...
<div class="wpcpo-item-line">
    <label>
        ...
        <input type="text" class="checkbox-show"
               name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][value]' ); ?>"
               value="<?php echo esc_attr( $this->get_field_value( 'value' ) ); ?>"/>
    </label>
</div>
<?php if ( $type === 'checkbox' || $type === 'image-checkbox' || $type === 'color-checkbox' ) { ?>
    <div class="wpcpo-item-line">
        <label>
            <?php esc_html_e( 'Limit', 'wpc-product-options' ); ?>
            <input type="number" min="0" max="1000" step="1"
                   name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][limit]' ); ?>"
                   value="<?php echo esc_attr( $this->get_field_value( 'limit' ) ); ?>"/>
            <em class="description"><?php esc_html_e( 'The maximum number of options can be selected.', 'wpc-product-options' ); ?></em>
        </label>
    </div>
<?php }
